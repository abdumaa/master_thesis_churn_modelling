...
\textit{Boosted Classification Trees}
...
The idea of boosted classification trees is to have a sequence of dependent base learners (trees), which improve in terms of accuracy where it is most needed. Thereby, the base learners are constructed in a shallow shape, allowing for a bias in their accuracy. The resulting loss of each tree is then fitted by the preceeding trees. So the procedure can be summarized by these steps: \\
1. Set first fit to $\hat{F}(X) := 0$ and loss to $L := \sum_{i=1}^{N} l(C^{(i)}, \hat{F}(X^{(i)}))$ \\
2. For the sequence of trees $b=1,..,B$ repeat: \\
2.1 Fit shallow tree $\hat{f}_{b}(X)$ which minimizes the Loss $L = \sum_{i=1}^{N} l(C^{(i)}, \hat{F}_{b-1}(X^{(i)}) + \hat{f}_{b}(X^{(i)}))$ \\
2.2 Update predicition to: $\hat{F}(X) := \hat{F}(X) + \lambda\hat{f}_{b}$ \\
2.3 Update loss to $L := \sum_{i=1}^{N} l(C^{(i)}, \hat{F}(X^{(i)}))$ \\
3. Final prediction is then: $\hat{C} = \hat{f}(X) = \sum_{b=1}^{B}\lambda\hat{f}_{b}(X)$ \\

The hyperparameter $\lambda$ is called learning rate and set to a small number to learn slowly and to avoid overfitting. Also notice, that the loss function defined here is the objective loss function used for gradient boosting and is defined differently than the loss function, when constructing a single tree. The objective loss function we will use for Gradient Boosting will be the log-loss derived in (\ref{loss_fct}). This allows us to handle the class imbalance as well as add regularization.\\
