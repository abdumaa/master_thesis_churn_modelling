...
The purpose is to build a model which can be used at any time $t$ of the year besides in January the 1st to predict the probability of
a costumer leaving the company in the next period of time $(t, t+s]$. \\
To provide a model with maximized utility for production one would want to keep $s$ as small as possible. However we will see that
having a small $s$ will decrease the accuracy of our models (drastically?). With a smaller period the classes of the data become more
imballanced, creating a higher challange of preprocessing the training data the correct way.
Figure 1 (richtiger Verweis) illustrates how this probability on costumer level can be decomposed using the Events A (Accident), N
