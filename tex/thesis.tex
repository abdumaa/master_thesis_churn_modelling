...
\begin{equation} \label{loss_fct}
    L(C, \widehat{C}) = -\sum_{i=1}^{N}w_{1}C^{(i)}log(\widehat{C^{(i)}}) + w_{0}(1 - C^{(i)})log(1 - \widehat{C^{(i)}}) + \lambda h(\theta)
\end{equation}
...
The hyperparameter $\lambda$ is called learning rate and set to a small number to learn slowly and to avoid overfitting. Also notice, that the loss function defined here is the objective loss function used for gradient boosting and is defined differently than the loss function, when constructing a single tree. The objective loss function we will use for Gradient Boosting will be the log-loss derived in (\ref{loss_fct}). This allows us to handle the class imbalance (but we will leave out the regularization term for now).\\

To apply the log-loss function in (\ref{loss_fct}) we need to deliver the gradient as well as the hessian of this function. Herewith the lightgbm-package again aims to improve time-cost as is it takes the second order Taylor Expansion to compute the optimal loss minimizing $\hat{f}_{b}(X)$ in each iteration $b=1,...,B$. This can be computed as follows: \\

The main cost with boosted trees is learning the single trees, and the most time-consuming part in that is to find the best split-points. Therefore we will use the package lightgbm in our prediction model, developped by (XY), which aims to tackle this issue. The so called histogram-based approach is to bin the features which creates high efficiency gains during searching for the optimal splits, especially for continuous variables. \\
Leaf-wise vs. Level-wise learning ? \\



There are a lot of hyperparameters which allow to tune the tree structure. Most important are the maximum depth and the minimum number of samples in a node (and XY). In Figure (1) is an overview of the hyperparameters we will focus on and the corresponding values we will use for grid search.

\begin{table}[]
    \begin{tabular}{llll}
    \hline
    \multicolumn{1}{l|}{}                             & \multicolumn{1}{l|}{Hyperparameter}      & Description                                             & \multicolumn{1}{l|}{Grid Search Values}      \\ \hline
    \multicolumn{1}{c|}{\multirow{7}{*}{\begin{sideways} Boosting \end{sideways}}}    & \multicolumn{1}{l|}{learning rate}       & Boosting learning rate $\lambda$                        & \multicolumn{1}{l|}{Shrinking learning rate} \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{n\_estimators}       & Number of boosted Trees B                               & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{objective}           & Objective and/or objective loss function                & \multicolumn{1}{l|}{Custom loss function}    \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{subsample}           & Subsample ratio of training data used                   & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{subsample\_freq}     & Frequency of the usage of subsample                     & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{reg\_alpha}          & L1 regularization term on weights                       & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{c|}{}                             & \multicolumn{1}{l|}{reg\_lambda}         & L2 regularization term on weights                       & \multicolumn{1}{l|}{}                        \\ \hline
    \multicolumn{1}{l|}{\multirow{5}{*}{\begin{sideways} Single Tree \end{sideways}}} & \multicolumn{1}{l|}{num\_leaves}         & Maximum tree leaves                                     & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{l|}{}                             & \multicolumn{1}{l|}{subsample\_for\_bin} & Number of samples for constructing bin                  & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{l|}{}                             & \multicolumn{1}{l|}{min\_split\_gain}    & Minimum loss reduction for further split on a leaf node & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{l|}{}                             & \multicolumn{1}{l|}{min\_child\_samples} & Minimum number of data needed in a new region           & \multicolumn{1}{l|}{}                        \\
    \multicolumn{1}{l|}{}                             & \multicolumn{1}{l|}{colsample\_bytree}   & Subsample ratio of columns constructing each tree       & \multicolumn{1}{l|}{}                        \\ \hline
    \end{tabular}
    \end{table}
