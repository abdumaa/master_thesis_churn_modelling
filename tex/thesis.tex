...
Before presenting the results we want to make some additional notational remarks. We denote the best model resulting from M-HPTL for a specific iteration of S-HPTL by: "gtb\_\{sampling method\}\_\{custom loss parameters\}\_\{quotation variables\}". So for example "gtb\_d0.1\_a0.6g2\_wquot" represents the best model with a downsampled dataset with factor $0.1$, with a focal loss implementation of $\alpha=0.6$ and $\gamma=2$ and uncluding the best quotation variables. \\
...
\begin{table}
    ...
    Custom Loss & $\alpha$: [None, 0.6, 0.7, 0.8], $\gamma$: [None, 1, 2, 5] \\
